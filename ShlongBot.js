...
client.on("message", message => {
  ...
  if (message.content === call + "avocado") {
    message.channel.send(
      "Avocado is " + status[Math.floor(Math.random() * status.length)] + "!"
    );
  }
  if (message.content === call + "freeleon") {
    ...
    message.channel.send(
      `${message.author.username} thinks ikaros should get his role back!`
    );
    ...
    const collectorY = message.createReactionCollector(filterY, {
      time: 15000
    });
    const collectorN = message.createReactionCollector(filterN, {
      time: 15000
    });
  ...
  if (message.content === call + "drink") {
    message.channel.send("Goaßmass saffa! :beers:"); //beer emoji
  }
